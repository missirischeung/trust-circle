...
const Dashboard: React.FC<DashboardProps> = ({
  ...
  const handleSendDeadlineReminder = async () => {
    try {
      ...
      const {
        error
      } = await supabase.functions.invoke('send-deadline-reminder', {
        body: {
          message: 'deadline_reminder'
        }
      });
      if (error) {
        throw error;
  ...
  const getRoleDisplayName = (role: string) => {
    switch (role) {
      case "admin":
        return "Administrator (Level A)";
      case "partner":
        return "Local Partner (Level B)";
      case "agent":
        return "Field Agent (Level C)";
      default:
        return role;
    }
  };
      ...
      <div className="container mx-auto px-4 py-6">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          <TabsList className="grid w-full grid-cols-4 lg:w-auto lg:grid-cols-none lg:inline-flex">
            ...
            {user.role !== "admin" && <TabsTrigger value="submit" className="flex items-center gap-2">
          ...
          {user.role !== "admin" && <TabsContent value="submit">
                  ...
                  <CardContent className="space-y-4">
                    <div className="bg-muted/50 p-4 rounded-lg">
                      ...
                      <p className="text-sm text-muted-foreground mb-3">
                       ...
                       <Button onClick={handleSendDeadlineReminder} className="flex items-center gap-2">
